    ...
    for (dir, id_, name) in ids:
        ...
        while True:
            ...
            if htmlpage is None:
                ...
                with open(os.path.join(gamedir, 'reviews-%s.html' % page), 'w', encoding='utf-8-sig') as f:
                    htmlpage = htmlpage.decode()
                    #if endre.search(htmlpage) or index > 9:
        ...
        with open(donefilename, 'w', encoding='utf-8-sig') as f:
            pass
...
def main():
    ...
    parser.add_argument(
        '-p', '--pause', help='Seconds to wait between http requests. Default: 0.5', required=False, default=0.3,
        type=float)
    parser.add_argument(
